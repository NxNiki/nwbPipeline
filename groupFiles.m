...
%    files will be grouped based on their name pattern. Files in difference
...
% 
%    eventFileNames - cell [n]. '.env' files to be combined. If not event
...
for i = 1:length(directories)
    ...
    % .ncs files:
    filenames = getFileNames(directories{i}, '*.ncs', ignoreFilesWithSizeBelow); 
    ...
    fileSuffix(idx) = cellfun(@(x) ['_', x], fileSuffix(idx), 'UniformOutput', false);
    ...
    for j = 1:size(iGroupFileNames, 1)
        % remove file if it does not exists:
        for k = 1:length(fileSuffix)
            if ~exist(iGroupFileNames{j,k}, 'file')
                iGroupFileNames{j,k} = '';
                sprintf('file does not exist: %s\n', iGroupFileNames{j, k});
            end
        end

        if orderByCreateTime && length(fileSuffix)>1
            disp("groupFiles: order files by create time.");
            createTimes = cell(length(fileSuffix), 1);
            for k = 1:length(fileSuffix)
                [~, ~, createTime, ~] = Nlx_getStartAndEndTimes(iGroupFileNames{j,k});
                createTimes(k) = createTime;
            end
            [~, order] = sort(createTimes);
            iGroupFileNames(j,:) = iGroupFileNames(j, order);
        elseif length(fileSuffix)>1
            warning("groupFiles: order files by file name. Make sure the order is correct by checking groupFileNames!")
            iGroupFileNames(j,:) = sort(iGroupFileNames(j,:));
        end
    end
    ...
    if length(filenames) > 1
        ...
        startTimeStamp = cell(length(filenames), 1);
        for j = 1:length(filenames)
            [~, ~, startTimeStamp{j}, ~] = Nlx_getStartAndEndTimes(filenames{j});
        end
        [~, order] = sort(startTimeStamp);
