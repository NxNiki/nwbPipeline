...
plot(fxx, pxx, 'LineWidth', .3, 'LineStyle', '-', 'Color', [0.1, 0.7, 0.2, .9]);
...
if ~isempty(signal2)
    ...
    hold on;
    plot(fxx, pxx, 'LineWidth', .3, 'LineStyle', '-', 'Color', [0.7, 0.1, 0.2, transparency]);
...
if ~isempty(signal3)
    ...
    hold on;
    plot(fxx, pxx, 'LineWidth', .3, 'LineStyle', '-', 'Color', [0.2, 0.1, 0.7, transparency]);
...
pxx = pxx(101:floor(length(pxx)/4));
fxx = fxx(101:floor(length(fxx)/4));
