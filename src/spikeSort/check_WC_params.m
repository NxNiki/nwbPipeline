...
if isfield(par,'features') && strcmp('features','wav')
    ...
    if isfield(par, 'scales') &&  isfield(par, 'w_post') &&  isfield(par, 'w_pre')
         L = wmaxlev(par.w_pre+par.w_post, 'haar');
         if L < par.scales
             error('[par.scales] exceeds maximum wavelet decomposition level for a waveform of length [par.w_pre+par.w_post]')
         end
    end
